...
def create_timepoint_boxplot_LME_dti(df, parameter, result, timepoints=['ultra-fast', 'fast', 'acute', '3-6mo', '12-24mo'], 
    ...
    if fixed_only is False and fixed_effects_result is not None:
        ax.set_title(f'Linear Mixed and Fixed Effects Model Predictions of {param_name} Difference by Timepoint', fontweight='bold')

    # Add grid for y-axis only
    ax.grid(True, axis='y', linestyle='-', alpha=0.3)
    
    # Move the legend to a better position
    ax.legend(handles=legend_elements, title='Region', bbox_to_anchor=(1.2, 1),loc='upper right')
        ...
        if count > 0:
            if parameter == 'fa':
                ax.text(i, ax.get_ylim()[0] * 1.15, f"n={count}",
                    ha='center', va='bottom', fontsize=12) #, fontsize=10
            else:
                ax.text(i, ax.get_ylim()[0] * 1.35, f"n={count}",
                    ha='center', va='bottom', fontsize=12) #, fontsize=10
    ...
    for curr_region in regions_to_analyze:
        ...
        for tp in timepoints:
            ...
            for current_col, baseline_col in ring_mapping.items():
                ...
                if valid_indices.sum() > 0:
                    # Calculate difference Craniectomy - control
                    diff_values = current_values[valid_indices] - baseline_values[valid_indices]
...
if __name__ == '__main__':
    ...
    model3 = smf.mixedlm("GOSE_COMBI ~ fa_anterior_diff + area_diff", 
                        ...
                        groups=fa_outcome_merged["patient_id"])
    result3 = model3.fit()
    print("Model 3: Combined Anterior FA Difference and Herniation Area model")
    print_lme_summary_precise(result3,precision=8)
    # sys.exit()



    # Model 1: MD anterior diff predicting GOSE outcome
    model1 = smf.mixedlm("GOSE_COMBI ~ md_anterior_diff", 
                        data=md_outcome_merged, 
                        groups=md_outcome_merged["patient_id"])
    result1 = model1.fit()
    print("Model 1: MD anterior -> GOSE")
    # print(result1.summary())
    print_lme_summary_precise(result1,precision=8)
   

    # sys.exit()
    ## NO NEED TO DO AREA DIFF MODEL AGAIN
    # # Model 2: Area diff predicting GOSE outcome  
    # model2 = smf.mixedlm("GOSE_COMBI ~ area_diff", 
    #                     data=fa_outcome_merged, 
    #                     groups=fa_outcome_merged["patient_id"])
    ...
    # Model 3: Combined model (optional - to compare)
    model3 = smf.mixedlm("GOSE_COMBI ~ md_anterior_diff + area_diff", 
                        data=md_outcome_merged, 
                        groups=md_outcome_merged["patient_id"])
    result3 = model3.fit()
    print("Model 3: Combined Anterior MD Difference and Herniation Area model")
    print_lme_summary_precise(result3,precision=8)
