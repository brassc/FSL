...
# Define base paths
SOURCE_BASE="/home/cmb247/rds/hpc-work/Feb2025_working"
DEST_BASE="/home/cmb247/rds/hpc-work/April2025_DWI"

echo "Source base path: $SOURCE_BASE"
...
for patient_dir in "$SOURCE_BASE"/*; do
    # Check if it's a directory and contains numbers and letters
    if [[ -d "$patient_dir" && $(basename "$patient_dir") =~ [a-zA-Z] && $(basename "$patient_dir") =~ [0-9] ]]; then
        ...
        for bet_file in $modified_bet_files; do
            ...
            if [[ $bet_filename =~ T1_bias-Hour-([0-9]{5})_ ]]; then
                timepoint="${BASH_REMATCH[1]}"
                echo " Detected timepoint: $timepoint"
                
                # Create destination directory for dynamic Hour dirs
                matching_dirs=("$DEST_BASE/$patient_id/Hour-${timepoint}_E"*"_dwi")
                
                # Check if any matching directory exists
                if [ ${#matching_dirs[@]} -gt 0 ] && [ -d "${matching_dirs[0]}" ]; then
                    matched_dir="${matching_dirs[0]}"
                    echo " Found destination Hour directory: $matched_dir"
                else
                    echo " WARNING: No matching destination directory found for timepoint $timepoint"
                    continue
                fi


                dest_dir="$matched_dir/T1_space_bet"
